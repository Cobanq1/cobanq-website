...
03-components.md           Interactive behaviour (nav, sliders, calculators, forms)
...
screenshots/
  desktop/                 Full-page captures at 1440px — 23 pages
  mobile/                  Full-page captures at 390px — 23 pages
  onboarding/              All 8 screens of the CoPay signup flow
```

## The fastest way to understand the site

1. Open `screenshots/desktop/home.png`, then `solutions.png`, then
   `copay-send-money.png`. That is the shape of the whole thing.
2. Skim `07-outstanding.md` so you know what is placeholder before you
   quote or start.
3. Run the reference build (below) and click around — it is faster than
   reading any spec.
...
server-side code, no database, no API. Nothing in the current site needs
a backend except the enquiry forms — see `05-forms-and-email.md`.
...
- **CoPay** — the consumer remittance product, "powered by CoBanq".
  Lives at `/send-money` and `/onboarding/remittance`. Has its own
  palette (near-black navy), its own serif type treatment, and its own
  logo. It should feel like a sibling brand, not a section.
...
plain JavaScript module of nested objects and arrays; it will read fine
in any editor and converts trivially to JSON, YAML, or CMS fields.
