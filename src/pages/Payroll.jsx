...
import { ArrowRight, Users, Repeat, BarChart3, ShieldCheck } from "lucide-react";
import { payroll } from "../content";
import Flag from "../components/Flag";

const icons = { Users, Repeat, BarChart3, ShieldCheck };

export default function Payroll() {
  const { openGetStarted } = useOutletContext();
  const { mockup } = payroll;
          ...
          <div className="relative mx-auto w-full max-w-sm lg:justify-self-end">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur">
                ...
                {mockup.rows.map((row) => (
                  ...
                  >
                    <span className="flex items-center gap-2.5 text-sm text-white/80">
                      <Flag code={row.countryCode} className="h-5 w-7 rounded" />
                      {row.name}
                    </span>
              ...
              <button
                type="button"
                onClick={openGetStarted}
                className="mt-4 w-full rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 py-2.5 text-sm font-semibold text-white"
              >
                Send payroll
              </button>
            </div>
